...
public:
  ...
  RAIIThread(std::thread &&t)
      : t_{std::move(t)}, dtor_action_{DtorAction::Join} {}
  RAIIThread(std::thread &&t, DtorAction dtor_action)
      : t_{std::move(t)}, dtor_action_{dtor_action} {}
  ~RAIIThread() {
...
realAsync(Callable &&callable, Args &&...args) {
  return std::async(std::launch::async, std::forward<Callable>(callable),
